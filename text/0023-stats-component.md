...
## Advanced stats
Dataset creators can opt into calculating more advanced stats, stats that take more computing power then the default stats but are very useful for some datasets, by using an `--advanced-stats` flag on save. 
...
Since these calculations take much more memory to preform, they should come with a warning for datasets over 200MB.

## Custom stats

Dataset creators can manually add customs stats to a dataset, the same way a creator can add a Meta or Structure, using the `dataset.yaml` file. Qri can know if these additions were manually added, and can label them as such.

This is out of the scope of this current RFC, but Qri should implement a way for users to add their own custom stats during a transform. We should create a starlark module called `stats`, and a function on the dataset object `ds.set_stats`. These stats would be more auditably and generated by the transform, and therefore not considered manual.
...
## iterating through the body
To implement the default stats, we need a new entry reader that will iterate through the body, and calculate the stats as we iterate. 

```go
// DefaultStatsGenerator wraps a dsio.EntryReader, on each call to read stats will update
// it's internal statistics
type DefaultStatsGenerator struct {
  r     dsio.EntryReader
  stats statGenerator
  adv   boolean
}
...
func newStatGenerator(val interface{}, adv boolean) {
  // switches on the val type, returning a specific kind 
  // of `statsGenerator`, depending on val's type
}
```

We then need a `objectStatGenerator`, `arrayStatGenerator`, `numericStatGenerator`, `stringStatGenerator`, and `booleanStatGenerator`, each of which contains the fields needed to calculate their specific stats, and returns a `stats` of a different shape.

Any time we create a `newStatGenerator`, we switch on the type, and return a `statGenerator` of that type. 

Each call to `AddEntry` on a specific `statGenerator`, adds that entry to the stat calculations. The `statGenerator.Stats` function, knows how to zip up the underlying calculations and return it as a `stats`.

## dataset & qri changes

We need a new struct in the `dataset` package called `Stats`. We also need to add stats to the create dataset process, we need load and dereference functions in dsfs, add `stats` to the list of Package files, etc.
